...
def test_edit_form_does_not_expose_steps_or_ingredients(client: TestClient) -> None:
    """Body content changes go through the review gate, never a raw form."""
    recipe_id = _first_recipe_id(client, "Луковый суп")
    form = client.get(f"/recipes/{recipe_id}/edit").text

    assert 'name="steps"' not in form
    assert 'name="ingredients"' not in form
